pipeline {
	...
	stages {
		stage('Checkout') {
			steps {
				sh 'echo checkout'
				checkout scmGit(branches: [[name: '*/main']], extensions: [], userRemoteConfigs: [[credentialsId: 'GitHub', url: 'https://github.com/thuerand/DevOps-06-Quality']])
			}
		}
        stage('Build') { 
            steps {
        ...
        stage('Docker') {
            steps {
                sh '''
                    ...
                    docker build -t thuerand/devopsdemo .
                '''
            }
        }
	}
